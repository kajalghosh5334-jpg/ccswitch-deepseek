...
function buildChatBody(body) {
  ...
  const extraSystemContent = systemMessages.map(m => m.content).filter(Boolean).join("\n\n");
  if (extraSystemContent) {
    instructions = instructions + "\n\n" + extraSystemContent;
  }

  const nonSystemMessages = messages.filter(m => m.role !== "system");
  nonSystemMessages.unshift({ role: "user", content: "<system>\n" + instructions + "\n</system>" });
  messages.length = 0; messages.push(...nonSystemMessages);
...
const server = http.createServer(async (req, res) => {
  ...
  if (req.method === "POST" && (url.pathname === "/v1/responses" || url.pathname === "/responses")) { try { const raw = await readBody(req); const body = JSON.parse(raw); const { chatBody, stream, messages } = buildChatBody(body); console.log("[DUMP]", JSON.stringify(chatBody.messages.map(m => ({role:m.role,content:(m.content||"").slice(0,80)})))); const sk = sessionKey(body);
